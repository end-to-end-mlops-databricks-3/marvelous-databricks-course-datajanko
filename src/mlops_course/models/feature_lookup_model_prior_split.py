...
class FeatureLookupModel:
    ...
    def __init__(self, config: ProjectConfig, tags: Tags, spark: SparkSession) -> None:
        ...
        self.model_name = f"{self.catalog_name}.{self.schema_name}.cs_go_model_feature_lookup_prior_split"
        self.validation_start = self.config.validation_start_day
        self.tags = tags.dict()
        self.overall_feature_table_name = (
            f"{config.catalog_name}.{config.schema_name}.overall_winning_shares_fe_parsed_timestamp"
        )
        self.per_map_feature_table_name = (
            f"{config.catalog_name}.{config.schema_name}.per_map_winning_shares_fe_parsed_timestamp"
        )
    ...
    def load_data(self) -> None:
        ...
        self.validation_set = self.spark.table(f"{self.catalog_name}.{self.schema_name}.validation_set")
        ...
        def lookup(df):  # noqa
            return self.fe.create_training_set(
                ...
                exclude_columns=[
                    "date"
                ],  # we exclude this for now as we use pandas for train/validation split. If we conduct this on the spark level, we'd need to make the above training set a function and call it twice as this is only available once the model is registered
            )
        ...
        self.train_set = lookup(self.training_set)
        train_df = self.train_set.load_df().toPandas()
        valid_df = lookup(self.validation_set).load_df().toPandas()

        self.X_train = train_df.drop(["date", self.target], axis=1)
        self.y_train = train_df[self.target]
        self.X_valid = valid_df.drop(["date", self.target], axis=1)
        self.y_valid = valid_df[self.target]
